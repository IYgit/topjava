...
//        return jdbcTemplate.queryForObject("SELECT * FROM users WHERE email=?", ROW_MAPPER, email);
    ...
    public List<User> getAll() {
        // 1. отримання ролей
        Map<Integer, Set<Role>> map = new HashMap<>(); // map(userId, Set<Role>)
        // rs - ResultSet
        jdbcTemplate.query("SELECT * FROM user_roles", rs -> {
            // computeIfAbsent(int key, Function f); EnumSet.noneOf(Role.class) - створення пустого EnumSet<Role>
            map.computeIfAbsent(rs.getInt("user_id"), userId -> EnumSet.noneOf(Role.class)) // метод повертає val map -> Set<Role>
                    .add(Role.valueOf(rs.getString("role"))); // додавання ролі в Set<Role>
        });
        // 2. отримання користувачів
        List<User> users = jdbcTemplate.query("SELECT * FROM users ORDER BY name, email", ROW_MAPPER);
        // 3. додавання ролей користувачам
